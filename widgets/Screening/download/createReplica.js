    ...
    _exportFileUsingCreateReplica: function (parentLayerURL, currentLayer) {
      ...
      requestHandle.then(lang.hitch(this, function (response) {
        var statusUrl = response.statusUrl;
          var status = setInterval(lang.hitch(this, function () {
            ...
            statusHandle.then(lang.hitch(this, function (statusResponse) {
              if (statusResponse.status === "Completed") {
                clearInterval(status);
                this.onRequestSucceeded(createReplicaDef, statusResponse);
              } else if (statusResponse.status === "CompletedWithErrors" || statusResponse.status === "Failed ImportChanges") {
                clearInterval(status);
                if (this.errMessageString) {
                  this.errMessageString += ", ";
                }
                this.errMessageString += currentLayer.layerNames.join(", ");
                this.onRequestFailed(createReplicaDef, statusResponse);
              }
